...
class RealValCNNLineDet(nn.Module):
    ...
    def __init__(self, activation="relu", input_dim=2, input_size=[12, 92, 112],
                 ...
                 dtype=torch.float64, normalization=None,
                 increasing_filters=True, **kwargs):

        super(RealValCNNLineDet, self).__init__()
        ...
        filters = []
        if increasing_filters:
            for i in range(1, num_layer // 2 + 1):
                filters.append(first_filters * i)
                filters.append(first_filters * i)
            if num_layer % 2 == 1:
                filters.append(first_filters * (num_layer // 2 + 1))
        else:
            for i in range(1, num_layer // 2 + 1):
                filters.append(first_filters // i)
                filters.append(first_filters // i)
        ...
        if input_size[0] == 12:
            ...
            if num_layer > 3:
                for i in range(3, nr):
                    pool_kernel_size.append((4, 1, 2))
                    pool_stride.append((4, 1, 2))
